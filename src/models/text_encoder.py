...
class TextMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask: (batch_size, seq_length) - 1 for tokens to attend, 0 for padding
        Returns:
            (batch_size, seq_length, hidden_dim)
        """
        ...
        # Compute Q, K, V
        q = self.query(hidden_states).reshape(B, N, self.num_heads, self.head_dim).transpose(1, 2)
        k = self.key(hidden_states).reshape(B, N, self.num_heads, self.head_dim).transpose(1, 2)
        v = self.value(hidden_states).reshape(B, N, self.num_heads, self.head_dim).transpose(1, 2)
...
class TextEncoder(nn.Module):
    ...
    def forward(
        ...
        Returns:
            cls_token: (batch_size, hidden_dim) - pooled representation (if use_cls_token)
            sequence_output: (batch_size, seq_length, hidden_dim) - all token representations
        """
        # Get embeddings
...
class SimpleTokenizer:
    ...
    def encode(self, text: str, max_length: int = 512) -> dict:
        ...
        input_ids.extend([min(ord(c) % (self.vocab_size - 10), self.vocab_size - 1) for c in text[:max_length - 2]])
