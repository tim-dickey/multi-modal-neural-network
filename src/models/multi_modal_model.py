...
class MultiModalModel(nn.Module):
    ...
    def __init__(
        ...
        if use_double_loop:
            self.double_loop_controller = create_double_loop_controller(double_loop_config)
    ...
    def forward(
        ...
        if self.use_double_loop and self.training:
            if current_loss is not None and current_accuracy is not None and gradient_norm is not None:
        ...
        if hasattr(self.task_head, 'forward'):
            ...
            if task_name == 'contrastive' or \
               (hasattr(self.task_head, '__class__') and 
                self.task_head.__class__.__name__ == 'ContrastiveHead'):
