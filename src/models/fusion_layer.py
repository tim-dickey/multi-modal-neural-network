...
class CrossModalAttention(nn.Module):
    ...
    def forward(
        ...
        q = self.query(query_features).reshape(B, N_q, self.num_heads, self.head_dim).transpose(1, 2)
        k = self.key(key_value_features).reshape(B, N_kv, self.num_heads, self.head_dim).transpose(1, 2)
        v = self.value(key_value_features).reshape(B, N_kv, self.num_heads, self.head_dim).transpose(1, 2)
...
class EarlyFusionLayer(nn.Module):
    ...
    def forward(
        ...
        vision_features = vision_features + self.modality_embed[0].unsqueeze(0).unsqueeze(0)
    ...
    def __init__(
        ...
        if fusion_type == 'early':
            ...
            self.fusion = LateFusionLayer(
                hidden_dim=hidden_dim,
                fusion_method=late_fusion_method,
                dropout=dropout
            )
        else:
